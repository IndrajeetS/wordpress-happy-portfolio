...
(function () {
  ...
  if (typeof wedoAjax === "undefined" || !wedoAjax.ajaxurl) {
    console.warn(
      "⚠️ wedo-custom: wedoAjax.ajaxurl not found. Ensure wp_localize_script provides it.",
    );
  }
  ...
  function initWedoTabsActual(isAjaxLoad = false) {
    ...
    // FIX: Use .wedo-tab-btn
    const initialBtn =
      root.querySelector(`.wedo-tab-btn[data-term="${initial}"]`) ||
      root.querySelector(".wedo-tab-btn");
    if (initialBtn) {
      // set visual state
  ...
  if (typeof window.wedoInitTabs !== "function") {
    ...
    window.wedoInitTabs = function (isAjaxCall = false) {
      ...
      try {
        if (typeof prev === "function") prev(isAjaxCall);
      } catch (e) {
        /* noop */
      }
    };
