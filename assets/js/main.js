...
const SPA = (function () {
  ...
  function initializePageContent(isAjax = false) {
    ...
    if (
      ...
      typeof window.wedoInitTabs === "function"
    ) {
  ...
  function initContactModal() {
    ...
    if (typeof jQuery !== "undefined") {
      jQuery(document).on(
        "click",
        "#close-contact-modal, #contact-modal",
        function (e) {
          if (
            e.target.id === "contact-modal" ||
            jQuery(e.target).closest("#close-contact-modal").length
          ) {
            e.preventDefault();
            e.stopPropagation();
            hideContactPopup();
      ...
      if (closeModalButton)
        closeModalButton.addEventListener("click", hideContactPopup);
  ...
  async function loadPage(pageSlug, fullPath, updateHistory = true) {
    if (pageSlug === SLUGS.CONTACT) return;
    ...
    try {
      ...
      if (contentArea)
        contentArea.innerHTML =
          '<p class="text-red-500 p-6">Error loading page.</p>';
      setActiveButton(pageSlug);
    }
  }
  ...
  function onDocumentClick(e) {
    ...
    if (buttonNav) {
      ...
      // Generic checking for external links so ANY custom configured links (e.g. LinkedIn, GitHub) will open correctly
    ...
    if (e.target.closest("#wpadminbar") || anchor.closest("#wpadminbar"))
      return;
...
// ----------------------------------------------------
window.initIframeLoaders = function () {
  const iframes = document.querySelectorAll("iframe");

  iframes.forEach((iframe) => {
    // Skip if already initialized
    if (iframe.hasAttribute("data-loader-init")) return;
    iframe.setAttribute("data-loader-init", "true");

    let parent = iframe.parentNode;

    // Find existing wp-block-embed wrapper or create one
    let wrapper = parent;
    if (
      ...
      wrapper.className =
        "iframe-wrapper relative rounded-md overflow-hidden shadow-sm";
    ...
    loader.className =
      "iframe-loader absolute inset-0 flex items-center justify-center z-10 w-full h-full rounded-md min-h-[200px]";
    loader.innerHTML = `
...
document.addEventListener("DOMContentLoaded", () => {
  if (typeof window.initIframeLoaders === "function") {
